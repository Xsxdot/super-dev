...
使用单库单表 SQLite 持久化，在 `service_id`、`run_id`、`timestamp` 上建索引，支持：
- 按 runId 区分每次启动会话
- 按 serviceId、runId 查询，不做关键词过滤（过滤在客户端做）
- 分页游标查询（`before` cursor 基于 log ID）

**不做分表/分库**：索引足以支撑百万行级别的查询，分表反而使跨服务聚合查询复杂化。

**批量写入缓冲**：实时日志不逐行 INSERT，在内存中积攒后每 100ms 或 50 条（取先到者）批量写入一次，大幅降低磁盘 I/O 压力。
