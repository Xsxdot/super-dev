...
func TestAgentStoreMigratesLegacyHostAgentIntoAgentsJSON(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(hostsPath, []byte(`[
		  {"id":"h1","name":"legacy","tags":[],"ssh_host_key_fingerprint":"SHA256:NeZJ8Xqm8k2RJoaxC7XMjjoXdw5R8TNigSr9hkWjK7A","pending_tunnel_invalidation_revision":"pending-revision","agent":{"token":"tok","transport":{"chain":[
	...
	assert.Equal(t, "pending-revision", hosts[0].PendingTunnelInvalidationRevision)
