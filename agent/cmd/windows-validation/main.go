...
func main() {
	...
	if options.FinalizeCleanup {
		if _, err := windowsvalidation.FinalizeCampaignCleanup(options.Run.ResultsRoot, options.CampaignID, options.CleanupReport); err != nil {
	...
	log.WithFields(map[string]any{"campaign_id": report.CampaignID, "status": report.Status, "tool_rows": len(report.ToolRows)}).Info("Windows 真实验证入口完成")
	// cleanup 在外层 PowerShell 卸载/恢复之后才能完成；进程退出码只表达本 lane 的功能状态。
	if report.FunctionalStatus != "PASS" {
		os.Exit(2)
	}
}
...
func parseOptions() cliOptions {
	...
	flag.StringVar(&options.CampaignID, "campaign-id", "", "campaign identity used only by --finalize-cleanup")
