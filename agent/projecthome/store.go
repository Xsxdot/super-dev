...
//
// 职责：
//   - 把每个项目当前归属的主机 ID 保存到 <DataDir>/project-homes.json
//   - 提供归属查询（HomeOf）、归属切换（SetHome）、按主机反查（ProjectsHomedOn）
//
// 边界：
...
//   - 文件形态 {"proj-id": "host-id"}；某项目缺席该 map 即视为归属本机
package projecthome
...
func (s *Store) SetHome(projectID, hostID string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	m, err := s.load()
	if err != nil {
	...
	log.Printf("[SuperDev] projecthome: 项目 %s 归属 %s→%s", projectID, old, hostID)
	return nil
}
