...
func TestAgentManualUninstallScriptsAreServedFromBundledVersionedResources(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			resp := httptestDo(t, app, http.MethodGet, "/api/agent-uninstall-scripts/"+tc.name, nil)
...
func TestAgentManualUninstallScriptRouteRejectsUnownedFiles(t *testing.T) {
	...
	resp := httptestDo(t, app, http.MethodGet, "/api/agent-uninstall-scripts/security.json", nil)
