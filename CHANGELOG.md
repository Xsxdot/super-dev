...
- Fixed native Windows Go language runtimes by building and launching `.exe` artifacts consistently, and exposed stopped service and deployment states through MCP as the canonical `stopped` value.
- Preserved deployment readiness/orchestration fields through MCP config upserts, isolated path-specific evidence redaction from global secrets, repaired the C++ and Kotlin Windows fixture wrappers, and made custom Rust/JVM toolchain paths explicit in the Windows validation input.
