...
1. 确保服务以 debug 模式在跑：`restart_service(deployment_id, mode="debug")`
   （服务已普通运行时用 restart；已停止用 `start_service(deployment_id, mode="debug")`）。
...
3. 看完恢复：`restart_service(deployment_id, mode="normal")` 回到普通运行。

注意：

- 不存在手动打开或关闭代码调试会话的工具；租约由 agent 内部管理，空闲自动回收。
- `debug_evaluate` 仍逐次审批（可读进程内状态）。
- 若服务未以 debug 模式运行，调试工具会报错并提示先 restart/start mode=debug，不会静默重启你的进程。
