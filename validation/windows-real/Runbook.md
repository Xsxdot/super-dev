...
1. 打开 **Settings → Hosts → New Host**，人工输入本轮专用 Linux 测试机的连接信息并保存；点击保存后，Desktop 会自动对目标地址发起一次只读 **SSH Host Key 指纹** 采集，并在 **SSH Host Key 指纹** 确认卡片中原样展示采到的 exact canonical `SHA256:<43-character raw Base64>` fingerprint（无首尾空白、无 Base64 padding），要求操作者显式点击确认后才写入配置——这是本轮唯一的信任根来源，操作者只核对确认卡片展示的值与预期主机一致，不得跳过确认直接默认信任。仅当目标主机在采集阶段不可达、无法直连时，才退回手填入口，改为录入事先从带外可信运维清单取得的 exact canonical fingerprint 作为兜底；禁止把该兜底手填的 raw fingerprint 写入验证 input、报告或证据。采集失败且没有带外可信 fingerprint 可兜底、UI 无法保存该值或后续连接未按 pin 验证时立即保持 `BLOCKED`。新建且只新建一个 lane-owned **dedicated Linux Host**，其 tags 列表必须精确为唯一非敏感 tag `superdev-validation-dedicated-resettable`；不得选择生产、个人或旧 campaign Host。UI 无法保存或回读这个 exact tag 时停止，不得继续。
